...
/**
 * The program combobox of step 2: a searchable select plus an Add button.
 * Search is server-side (`GET /programs?q=`, debounced) — the same endpoint
 * the filter panel's match count uses, so they can never disagree.
 *
 * Without `listExists === false`, the search stays global; pass `filters`
 * explicitly to override that. Selecting is separate from adding: pick a
 * program, read it back on the trigger, then press Add. Every row carries
 * `commune · region` under the label, since school names repeat across Chile.
 */
